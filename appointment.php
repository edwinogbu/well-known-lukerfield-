<?php
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        $advisor = trim($_POST["user-select-advisor"]);
        $date = trim($_POST["user-select-date"]);
        $message = trim($_POST["user-message"]);

        // Check that data was sent to the mailer.
        if ( empty($firstname) OR empty($lastname) OR empty($phone) OR empty($advisor) OR empty($date) OR empty($message) OR !filter_var($email, FILTER_VALIDATE_EMAIL)) {
